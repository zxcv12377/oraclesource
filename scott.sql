...
VALUES (7249, '홍길동', 'PRESIDENT', NULL, '2020-12-13', 5000, 1000, 10);
...
INSERT INTO DEPT_TEMP VALUES(30, 'DATABASE', 'SEUOL');
UPDATE DEPT_TEMP SET LOC = 'BUSAN' WHERE DEPTNO= 30;
DELETE FROM DEPT_TEMP WHERE DNAME = 'JAVA';
...
-- 2. 테이블 이름은 30 BYTE 이하
-- 3. 같은 사용자 안에서는 테이블명 중복 불가
-- 4. SQL 예약어는 테이블명으로 사용 불가
...
CREATE TABLE EXMA_EMP AS SELECT * FROM EMP;

-- 기존 테이블 구조만 이용해서 새 테이블 생성 (데이터 X)
CREATE TABLE EXMA_EMP AS SELECT * FROM EMP WHERE 1<>1;
...
ALTER TABLE EMP_DDL MODIFY EMPNO NUMBER(3,0);

-- 정도 또는 자리수를 축소할열은 비어있어야 합니다
--ALTER TABLE EMP_TEMP MODIFY EMPNO NUMBER(3,0);
...
-- MAMBER 테이블 생성
-- ID VARCHAR2(8) / NAME VARCHAR2(10)/ADDR VARCAHR(50)/EMAIL VARCHAR(30) / AGE NUMBER(4,0)
CREATE TABLE MEMBER(
	NO NUMBER(8) UNIQUE,
...
VALUES (MEMBER_SEQ.nextval, 'hong250', '홍길동', '서울시 종로구', 'hong123@naver.com', 24);

-- insert(remark X)
INSERT INTO MEMBER (NO, ID, NAME, ADDR, EMAIL, AGE)
VALUES (MEMBER_SEQ.nextval, 'hong1259', '홍길동', '서울시 종로구', 'hong123@naver.com', 24);


-- MEMBER 테이블 열 추가
-- BIGO VARCHAR(20) 열 추가
...
-- 시퀀스 : 오라클 DB에서 특정 규칙에 따른 연속 숫자를 생성하는 객체
-- 게시판 번호, 멤버 번호...
...
-- MINVALUE 1 (시퀀스에서 생성할 번호의 최소 값 : 기본값 NOMINVALUE(1-오름차순))
-- MAXVALUE 9999999999999999999999999999  (시퀀스에서 생성할 번호의 최대값 : (10^29)-1) 
-- NOCYCLE (1 ~ MAXVALUE 까지 다 발행된 후에 새로운 번호 요청시 에러 발생시킴) | CYCLE (번호가 다 발행된 후에 다시 처음으로 돌아감) 
-- CACHE 20 (시퀀스가 생성할 번호를 메모리에 미리 할당해 놓을 개수를 지정) | NOCACHE 
-- NOORDER (오름차순)

-- MEMBER테이블에 NO COLUMN 추가
ALTER TABLE MEMBER ADD NO NUMBER(20,0);

-- MEMBER의 NO 값은 시퀀스 값으로 입력하기
...
-- 제약조건(매우중요) : 테이블에 저장할 데이터를 제약하는 특수한 규칙
...
VALUES ('HONG123', NULL, '010-1234-4531');

INSERT INTO TBL_NOTNULL (LOGIN_ID, LOGIN_PWD, TEL)
VALUES ('HONG123', '', '010-1234-4531');
...
UPDATE TBL_NOTNULL 
SET TEL = '010-1234-5212'
WHERE LOGIN_ID = 'HONG123';

ALTER TABLE TBL_NOTNULL2 MODIFY(TEL CONSTRAINT TBLNN2_LOGIN_TEL NOT NULL);

-- 제약 조건 이름 변경
ALTER TABLE TBL_NOTNULL2 RENAME CONSTRAINT TBLNN2_LOGIN_TEL TO TBL_NN2_TEL_NN;
...
-- 데이터 무결성
-- 데이터베이스에 저장된 데이터의 정확성과 일치성 보장
-- DML 과정에서 지켜야 하는 규칙 (국룰)

INSERT INTO TBL_UNIQUE (LOGIN_ID, LOGIN_PWD, TEL)
VALUES ('HONG123', 'HONG123', '010-1234-4531');

INSERT INTO TBL_UNIQUE (LOGIN_ID, LOGIN_PWD, TEL)
VALUES (NULL, 'HONG123', '010-1234-4531');

ALTER TABLE TBL_UNIQUE MODIFY(TEL UNIQUE);

ALTER TABLE TBL_UNIQUE MODIFY(TEL NOT NULL);
...
-- DELETE시 주의점
-- 1) 참조하는 테이블(자식)의 데이터 삭제(변경)
-- 2) 참조대상이 되는 테이블(부모)의 데이터 삭제

-- 옵션 설정
-- ON DELETE CASCADE : 부모 삭제될때 자식도 같이 삭제
-- ON DELETE SET NULL : 부모 삭제될때 연결된 자식의 부모를 NULL로 변경
...
CREATE TABLE EMP_FK2(
	...
	SAL NUMBER(7,2) NOT NULL,
	COMM NUMBER(7,2),
	DEPTNO NUMBER(2,0) CONSTRAINT EMPFK_DEPTNO_FK2 REFERENCES DEPT_FK(DEPTNO) ON DELETE CASCADE
);
...
CREATE TABLE EMP_FK3(
	...
	SAL NUMBER(7,2) NOT NULL,
	COMM NUMBER(7,2),
	DEPTNO NUMBER(2,0) CONSTRAINT EMPFK_DEPTNO_FK3 REFERENCES DEPT_FK(DEPTNO) ON DELETE SET NULL
);
...
-- 체크 제약조건(SCOTT.SYS_C008431)이 위배되었습니다
INSERT INTO TBL_CHK
VALUES ('HONG123', '123', '010-1234-5341');

INSERT INTO TBL_CHK
VALUES ('HONG123', '1234', '010-1234-5341');
